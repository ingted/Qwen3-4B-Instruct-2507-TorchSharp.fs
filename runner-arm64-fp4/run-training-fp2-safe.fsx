...
let requireNativeSteQuantize () =
    ...
    if not ok then
        failwith
            "run-training-fp2-safe.fsx requires TS_Q4_STE_USE_NATIVE_QUANTIZE=1 to avoid fallback quantize OOM risk."

requireNativeSteQuantize ()

if scriptArgs.MaxTokens > 8 then
    failwithf "Safety cap: --max-tokens must be <= 8 in fp2-safe script, got %d" scriptArgs.MaxTokens
...
printfn "safety: single-turn only, first '!!!!' triggers fail-fast"
