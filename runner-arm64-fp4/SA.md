...
## 25. 2026-02-26 dat export 相容性分析（bridge 崩壞根因）
1. 觀測：`run-training-fp2 (fp2-model)` 可輸出 `我是 F#之神`，但同一份 dat 在 `run-training2`/bridge 路徑出現亂碼或 `!!!!`。
2. no-op 對照：`lr=0` 僅經過 export 後，bridge 仍異常，排除「訓練更新造成模型壞掉」。
3. 根因假設：export 寫回時把 `*.weight.scale` 強制改寫為 `elemType=5(float16)`，破壞 bridge/Q4Linear 對 scale 編碼的預期（原檔多為 `elemType=101`）。
4. 方向：export 必須保留原 entry 的 elemType 與 payload 佈局契約；至少先做到 `scale` 不被硬改 dtype。
5. 驗收標準：
   - 同一份 output dat 在 `run-training-fp2` 與 `run-training2` 對同 prompt 不再出現 `!!!!`/token=0 洪流。
   - no-op export dat 與 base dat 在 bridge 行為等價（語義可允許微差，但不得崩壞）。

## 26. 2026-02-26 修正後分析結論（export 已解 bridge 崩壞）
1. 根因確認：bridge 崩壞主因是 scale entry 寫回型別/編碼不相容，不是訓練更新本身。
2. 修正後 no-op 與有訓練更新 dat 都可在 bridge 正常生成，`!!!!` 洪流消失。
3. 仍存在的行為：whoami 強監督資料在高 lr 下會出現重複 token（過擬合），這是資料/訓練策略問題，不是 dat 格式問題。
