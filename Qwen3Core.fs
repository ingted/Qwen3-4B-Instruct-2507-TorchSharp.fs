...
module Qwen3Core =
  ...
  let buildBlockGraphNoCache
    ...
    =
    let inputNormStage =
      stageM "block.input_norm" (fun hidden ->
        rmsNormWeighted hidden norms.InputNorm cfg.RmsNormEps)

    let qkvContextStage =
      stageM "block.attn.qkv_context" (fun hiddenNorm ->
